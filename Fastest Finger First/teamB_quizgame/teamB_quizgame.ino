...
if(!digitalRead(pushButton)){
  char  replyPacket[] = "B1";  // a reply string to send back
...
if(!digitalRead(pushButton2)){
  char  replyPacket[] = "B2";  // a reply string to send back
    // send back a reply, to the IP address and port we got the packet from
    Udp.beginPacket(broadastIP, materUdpPort);
    Udp.write(replyPacket);
    Udp.endPacket();
   ...
   if(!digitalRead(pushButton3)){
    char  replyPacket[] = "B3";  // a reply string to send back
    // send back a reply, to the IP address and port we got the packet from
    Udp.beginPacket(broadastIP, materUdpPort);
    Udp.write(replyPacket);
    Udp.endPacket();
    ...
    if(!digitalRead(pushButton4)){
      char  replyPacket[] = "B4";  // a reply string to send back
    // send back a reply, to the IP address and port we got the packet from
    Udp.beginPacket(broadastIP, materUdpPort);
    Udp.write(replyPacket);
    Udp.endPacket();
